...
function applyJobPreset(jobKey) {
  ...
  if (jobKey === 'homemaker') {
    ...
    document.getElementById('rangeIncome').value = 0;
  } else if (parseInt(document.getElementById('numIncome').value) === 0) {
    document.getElementById('numIncome').value = 500;
    document.getElementById('rangeIncome').value = 500;
  }

  showToast(`職種を「${cfg.name}」に設定しました`);
  updateAll();
}
...
function updateAll() {
  ...
  const dataTotal = [0];
  const dataRealCost = [0]; // 実質負担額（元本 - 節税累計）
  const tableRows = [];
  latestTableData = [];
  ...
  for (let m = 1; m <= totalMonths; m++) {
    ...
    if (m % 12 === 0) {
      ...
      const totalBenefit = gains + accTaxSave; // 運用益 ＋ 節税累計
  ...
  // 表示更新
  document.getElementById('mainTaxSaveValue').innerText = fmtYen(totalTaxSave);
  document.getElementById('mainTotalValue').innerText = fmtYen(finalTotal);
  document.getElementById('chipAnnualTaxSave').innerText = `${fmtYen(annualTaxSave)}/年`;
...
function renderIdecoLineChart(labels, dataPrincipal, dataTotal, dataRealCost) {
  const ctx = document.getElementById('idecoChart').getContext('2d');
...
function shareIdecoOnX() {
  const taxStr = document.getElementById('mainTaxSaveValue').innerText;
  const totStr = document.getElementById('mainTotalValue').innerText;
